...
export const advisorUniversities: AdvisorUniversity[] = [
  ...
  },
  {
    ...
    courses: ["mba", "mca", "mcom", "ma", "ms-ds", "phd"],
  ...
  },
  {
    ...
    highlights: ["Research Excellence", "Global Mentorship"],
    courses: ["phd", "mba"],
    specializations: ["finance", "marketing", "operations", "statistics"],
    learningModes: ["self-paced", "weekend"],
    experienceLevel: ["1-3-years", "3-5-years", "5-10-years", "10-plus-years"]
  }
];

export function normalizeCourseSlug(idOrName: string | number | undefined): string {
  const s = String(idOrName || "").toLowerCase();
  if (s.includes("mba") || s === "5954" || s === "5960" || s === "5961" || s === "5962" || s === "22") return "mba";
  if (s.includes("mca") || s === "5955" || s.includes("computer")) return "mca";
  ...
  if (s.includes("phd") || s.includes("ph.d") || s.includes("doctorate") || s === "5965" || s === "5966" || s === "5967" || s === "5968" || s === "5969") return "phd";
  if (s.includes("dip") || s.includes("exec") || s === "5963" || s === "5964") return "pg-dip";
  return "mba";
}
...
export function matchUniversities(
  ...
  const slug = normalizeCourseSlug(courseId);

  // 1. Filter by course
  let eligibleUniversities = advisorUniversities.filter(
    (uni) => uni.courses.includes(slug) || uni.courses.includes(courseId)
  );

  // Fallback so matched colleges are always shown
  if (eligibleUniversities.length === 0) {
    eligibleUniversities = advisorUniversities;
  }
  ...
  const scoredUniversities: MatchedUniversity[] = eligibleUniversities.map((uni) => {
    ...
    if (!answers.specialization || answers.specialization === "help-me-decide") {
      ...
      specScore = 20; // partial match for related fields
    }
    score += specScore;
    ...
    return { ...uni, matchScore: Math.min(100, Math.max(70, score)) };
  });

  // 3. Sort descending and return top matches
